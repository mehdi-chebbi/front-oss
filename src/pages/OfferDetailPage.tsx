...
const OfferDetailPage = () => {
  ...
  useEffect(() => {
    if (offer) {
      ...
      setIsDeadlinePassed(currentTime >= deadline);
      
      // Close application form if deadline passes while it's open
      if (currentTime >= deadline && showApplicationForm) {
        setShowApplicationForm(false);
        alert(t('form.error.deadlinePassed'));
      }
    }
  }, [offer, currentTime, showApplicationForm]);
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                ...
                {!isExpired && (
                  <span className="px-3 py-1 text-sm font-semibold rounded-full bg-blue-100 text-blue-800">
                    {t('offer.countdown.expiresIn')}: {formatTimeRemaining(deadlineDate - currentTime, t)}
                  </span>
                )}
              </div>
