...
app.get('/offers/dashboard', auth, requireRole(['comite_ajout', 'comite_ouverture']), async (req, res) => {
  try {
    ...
    `, [now, now, now, now, now, now, ...params]);
...
app.get('/offers/:id', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`
      ...
      LEFT JOIN projects p ON o.project_id = p.id
      LEFT JOIN departments d ON p.department_id = d.id
      WHERE o.id = ? AND o.deadline > ?
    `, [req.params.id, now]);

    if (rows.length === 0) {
      return res.status(404).json({ error: 'Offer not found or deadline has passed' });
    }
...
app.post('/apply', uploadApplicantDynamic.any(), async (req, res) => {
  try {
    ...
    // Verify offer exists and get deadline
    const [offerRows] = await pool.query('SELECT id, title, type, created_by, deadline FROM offers WHERE id = ?', [offer_id]);
    if (offerRows.length === 0) return res.status(404).json({ error: 'Offer not found' });
    const offer = offerRows[0];

    // CRITICAL: Check if deadline has passed (exact time validation)
    const now = new Date();
